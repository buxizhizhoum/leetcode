...
class Solution(object):
        ...
        for i in range(len(matrix)):
            for j in range(len(matrix[0])):
                ...
                if not self.is_visited(i, j):
                    self.dfs(matrix, i, j, res, answer)
        return max(res)

    def dfs(self, matrix, x, y, res, answer):
        if not self.has_way_to_go(x, y, matrix):
            res.append(answer)
            # res = max(res, answer)
            # return res
            return
        ...
        for direction in self.directions:
            ...
            if self.in_matrix(new_x, new_y, matrix) \
                ...
                self.dfs(matrix, new_x, new_y, res, answer+1)
                # path_len = self.dfs(matrix, new_x, new_y, res, answer+1)
    ...
    def has_way_to_go(self, x, y, matrix):
        """
        the legal point around a certain point is all visited.
        :param x:
        :param y:
        :param matrix:
        :return:
        ...
        for direction in self.directions:
            ...
            if self.in_matrix(new_x, new_y, matrix) and not self.is_visited(new_x, new_y):
                # in this question, the new point should be larger
